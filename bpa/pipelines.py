...
class BbpaPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bpa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bpa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bpa (date, title, link, content)"
